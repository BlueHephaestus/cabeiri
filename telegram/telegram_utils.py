...
# telegram bot ---
TELEGRAM_TOKEN_ID = open('TELEGRAM_KEY','r').read()
TELEGRAM_CHAT_ID = '1056427289'  # chat-id
# ----------------
...
def notify(message):
    ...
    #msg = "Automatic message from host: '{hostname}', at: {ip}\n{separator}\n" \
          #"<b>MESSAGE: </b>\n<pre> {message} </pre>".format(separator=' -' * 10, message=message, hostname=hostname, ip=ip)

    #msg = f"message}"
    basic_notifier(logger_name='training_notifier',
